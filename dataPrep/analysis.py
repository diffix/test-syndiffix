...
def trans_avg_balance_by_cctype(df, df_mo, df_ct, fileName):
    ...
    sns.boxplot(x='card_type', y='avg_balance', hue='method', data=df_all, ax=axs[0])
    axs[0].set_xlabel('Card Type')
    axs[0].set_ylabel('Average Account Balance')
    axs[0].set_ylim(bottom=-20000, top=140000)
    axs[0].grid(axis='y')
    sns.boxplot(x='method', y='avg_balance', hue='card_type', data=df_all, ax=axs[1])
    axs[1].set_xlabel('Synthesis Method')
    axs[1].set_ylabel('Average Account Balance')
    axs[1].set_ylim(bottom=-20000, top=160000)
    axs[1].grid(axis='y')
    plt.tight_layout()
    synFilePath = os.path.join('plots', makeSynFileName(fileName, columns)+'.png')
    plt.savefig(synFilePath)
